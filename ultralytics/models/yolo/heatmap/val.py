...
class HeatmapValidator(BaseValidator):
    """A class extending the BaseValidator class for validation based on a Heatmap model.

    This class implements validation functionality specific to object Heatmap tasks, including metrics calculation,
    ...
    Attributes:
        ...
        metrics (DetMetrics): Object Heatmap metrics calculator.
        iouv (torch.Tensor): IoU thresholds for mAP calculation.
        niou (int): Number of IoU thresholds.
        lb (list[Any]): List for storing ground truth labels for hybrid saving.
        jdict (list[dict[str, Any]]): List for storing JSON Heatmap results.
        stats (dict[str, list[torch.Tensor]]): Dictionary for storing statistics during validation.

    Examples:
        >>> from ultralytics.models.yolo.detect import HeatmapValidator
        >>> args = dict(model="yolo26n.pt", data="coco8.yaml")
        >>> validator = HeatmapValidator(args=args)
        >>> validator()
    """

    def __init__(self, dataloader=None, save_dir=None, args=None, _callbacks=None) -> None:
        """Initialize Heatmap validator with necessary variables and settings.
    ...
    def init_metrics(self, model: torch.nn.Module) -> None:
        """Initialize evaluation metrics for YOLO Heatmap validation.
    ...
    def plot_predictions(
        ...
        Args:
            ...
            preds (list[dict[str, torch.Tensor]]): List of predictions from the model.
            ni (int): Batch index.
            max_det (int | None): Maximum number of Heatmaps to plot.
        """
        if not preds:
            return
    ...
    def save_one_txt(self, predn: dict[str, torch.Tensor], save_conf: bool, shape: tuple[int, int], file: Path) -> None:
        """Save YOLO Heatmaps to a txt file in normalized coordinates in a specific format.

        Args:
            predn (dict[str, torch.Tensor]): Dictionary containing predictions with keys 'bboxes', 'conf', and 'cls'.
            save_conf (bool): Whether to save confidence scores.
            shape (tuple[int, int]): Shape of the original image (height, width).
            file (Path): File path to save the Heatmaps.
        """
    ...
    def coco_evaluate(
        ...
        Performs evaluation using the faster-coco-eval library to compute mAP metrics for object Heatmap. Updates the
