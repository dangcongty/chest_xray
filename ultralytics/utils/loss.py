...
class HeatmapLoss(v8DetectionLoss):
    """Criterion class for computing training losses for YOLOv8 object detection."""
    ...
    # def z__call__(self, preds: Any, batch: dict[str, torch.Tensor], ct_classify = []) -> tuple[torch.Tensor, torch.Tensor]:
    #     """Calculate the sum of the loss for box, cls and dfl multiplied by batch size."""
    #     use_ct = self.hyp.use_ct
    #     if use_ct:
