...
__all__ = "OBB", "Classify", "Detect", "Pose", "RTDETRDecoder", "Segment", "YOLOEDetect", "YOLOESegment", "v10Detect", "Heatmap"
...
class SpatialSelfAttention(nn.Module):
    def __init__(self, in_channels):
        ...
        self.proj = nn.Conv2d(in_channels, 1, 1)  # tạo heatmap 1 channel
    ...
    def forward(self, x):
        ...
        q = self.q(x).view(B, -1, N)             # B x Cq x N
        k = self.k(x).view(B, -1, N)             # B x Ck x N
        v = self.v(x).view(B, -1, N)             # B x C  x N
        ...
        heatmap = self.proj(out)

        return heatmap

class Heatmap(Detect):
    def __init__(self, nc = 80, reg_max=16, end2end=False, ch = ()):
        ...
        #     nn.Sequential(Conv(x, max(x // 4, 16), 3), 
