...
class BaseDataset(Dataset):
    ...
    def get_image_and_label(self, index: int) -> dict[str, Any]:
        ...
        label['heatmap'] = np.load(self.hm_files[index]).astype(np.float16) if self.use_heatmap else np.zeros(shape=(label['img'].shape[0], label['img'].shape[1]))
        label['use_hm'] = self.use_heatmap
        label["ratio_pad"] = (
