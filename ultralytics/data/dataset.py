...
class YOLODataset(BaseDataset):
        ...
        with ThreadPool(NUM_THREADS) as pool:
            results = pool.imap(
                ...
                iterable=zip(
                    self.im_files,
                    self.label_files,
                    self.heatmap_files if self.use_heatmap else self.label_files,
                    repeat(self.prefix),
                    repeat(self.use_keypoints),
                    repeat(self.use_heatmap),
            ...
            for im_file, lb, shape, segments, keypoint, heatmaps, nm_f, nf_f, ne_f, nc_f, msg in pbar:
                nm += nm_f
                nf += nf_f
                ne += ne_f
                nc += nc_f
    ...
    def get_labels(self) -> list[dict]:
        ...
        if self.use_heatmap:
            import os
            sa, sb = f"{os.sep}images{os.sep}", f"{os.sep}heatmap{os.sep}"  # /images/, /labels/ substrings
            self.heatmap_files = [sb.join(x.rsplit(sa, 1)).rsplit(".", 1)[0] + ".npy" for x in self.im_files]
    ...
    def build_transforms(self, hyp: dict | None = None) -> Compose:
        ...
        transforms.append(
            Format(
                ...
                hm_scales=hyp.hm_scales if self.use_heatmap else [],
            )
        )
        return transforms
    ...
    def collate_fn(batch: list[dict]) -> dict:
        ...
        for i, k in enumerate(keys):
            ...
            if k in {"masks", "keypoints", "bboxes", "cls", "segments", "obb", 'heatmaps'}:
